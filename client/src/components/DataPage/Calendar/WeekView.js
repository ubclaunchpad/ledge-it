...
const DayLabels = () => {
  return (
    <View style={{display: 'flex', flexDirection: 'row', paddingLeft: scrollViewMargin, backgroundColor: theme.colors.primary}}>
    ...
    <ScrollView 
      ...
      {weekArray.slice(0, weekArray.length - 1).map((i, j) => {
        return (
          <TouchableOpacity 
            key={j} 
            style={i === selectedDate ? [styles.block, {backgroundColor: theme.colors.white}] : styles.block}
            onPress={() => setSelectedDate(i)}>
              <Text style={i === selectedDate ? [styles.text, {color: theme.colors.primary}] : 
      ...
      <TouchableOpacity
        ...
        style={weekArray[weekArray.length - 1] === selectedDate ? [styles.block, {backgroundColor: theme.colors.white, marginRight: 10}] : [styles.block, {marginRight: 10}]}
        onPress={() => setSelectedDate(weekArray[weekArray.length - 1])}>
          <Text style={weekArray[weekArray.length - 1] === selectedDate ? [styles.text, {color: theme.colors.primary}] : 
                                                [styles.text, {opacity: .75}]}>{weekArray[weekArray.length - 1].getDate()}</Text>
      </TouchableOpacity>
    </ScrollView>
