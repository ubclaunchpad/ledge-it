...
import { calendarWidthTotal } from './Calendar';
...
const MonthView = ({viewingExpenses, setViewingExpenses, calArr, setCalArr, monthEvents, setMonthEvents, categories, setSelectedDate, setupCalendar}) => {
  ...
  const loadInMultipleMonths = (earlierMonth, scrollTo) => {
    ...
    monthsApart = (laterMonth.getFullYear() - earlierMonth.getFullYear()) * 12 - earlierMonth.getMonth() + laterMonth.getMonth();
    for (let i = 1; i <= monthsApart; i ++) {
      arr.push(setupCalendar(new Date(laterMonth.getFullYear(), laterMonth.getMonth() - i, 1)));
    }
    setCalArr({arr: arr, scrollTo: scrollTo});
  }
  ...
  const goToMonth = (monthDate) => {
    ...
    const monthsApart = (monthDate.getFullYear() - month.getFullYear()) * 12 - month.getMonth() + monthDate.getMonth();
    if (monthDate < new Date(calArr[calArr.length - 1].year, calArr[calArr.length - 1].month, 1)) {
      ...
      scrollView.current.scrollTo({x: currentOffset - monthsApart * calendarWidthTotal, y: 0, animated: true});
    }
  ...
  const getTotal = () => {
    if (!(month.getFullYear() in monthEvents) || !(month.getMonth() in monthEvents[month.getFullYear()])) {
      return 0;
    } else {
      let t = 0;
