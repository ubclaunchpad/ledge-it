...
const buildWeekArray = (date, weeksBack = 2, weeksForward = 3) => {
  ...
  const firstDayOfArray = new Date(
    date.getFullYear(),
    date.getMonth(),
    firstDayOfSelectedWeek.getDate() - 7 * weeksBack,
  );
  for (let i = 0; i < 7 * (1 + weeksBack + weeksForward); i++) {
    const d = new Date(
      firstDayOfArray.getFullYear(),
      firstDayOfArray.getMonth(),
      firstDayOfArray.getDate() + i,
    );
    weekArray.push(d);
  }
  return weekArray;
    ...
    <View style={{ marginBottom: 20 }}>
      <TouchableOpacity
        ...
        <Entypo
          ...
          size={45}
          color="white"
          style={{ alignSelf: 'center', paddingTop: 20, backgroundColor: theme.colors.primary }}
        />
      </TouchableOpacity>
