...
const CalendarSubPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${URL}/incomes`, {}).then((res) => {
      const c = res.data;
      c.forEach((v, i) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setDateEvents(
        events[selectedDate.getFullYear()][selectedDate.getMonth()][selectedDate.getDate()],
      );
    } else {
      setDateEvents([]);
    }
