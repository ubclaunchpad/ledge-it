...
def get_budget(month: int, year: int, current_user: User = Depends(get_current_active_user)):
    if (
        budget := budget_collection.find_one({"month": month, "year": year, "email": current_user["email"]})
    ) is not None:
        return budget
...
def add_budget(budget: Budget = Body(...), current_user: User = Depends(get_current_active_user)):
...
def update_budget(month: int, year: int, budget: UpdateBudgetModel = Body(...), current_user: User = Depends(get_current_active_user)):
    ...
    if len(budget) >= 1:
        update_result = budget_collection.update_one(
            {"month": month, "year": year,  "email": current_user["email"]}, {"$set": budget}
        )
    ...
    if (
        existing_budget := budget_collection.find_one({"month": month, "year": year, "email": current_user["email"]})
    ) is not None:
...
def delete_budget(month: int, year: int, current_user: User = Depends(get_current_active_user)):
    delete_result = budget_collection.delete_one({"month": month, "year": year,  "email": current_user["email"]})
...
def update_budget_spent(month: int, year: int, change: float, current_user: User = Depends(get_current_active_user)):
    budget: Budget = budget_collection.find_one({"month": month, "year": year,  "email": current_user["email"]})
