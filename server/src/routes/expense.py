...
def get_expenses_by_month(year: int, month: int, current_user: User = Depends(get_current_active_user)):
    regex = compile(f"{year}-{f'0{month}' if month < 10 else month}-" + r"\d{2}")

    if (
        expenses := expense_collection.find({"date": {"$regex": regex}, "email": current_user["email"]}).sort(
            [("date", pymongo.DESCENDING), ("updated_at", pymongo.DESCENDING)]
        )
    ).count():
...
def get_expense_by_id(id, current_user: User = Depends(get_current_active_user)):
    if (expense := expense_collection.find_one({"_id": id, "email": current_user["email"]})) is not None:
        return expense
...
def create_expense(expense: AddExpense = Body(...), current_user: User = Depends(get_current_active_user)):
...
def update_expense(id, expense: UpdateExpenseModel = Body(...), current_user: User = Depends(get_current_active_user)):
    ...
    if len(expense) >= 1:
        update_result = expense_collection.update_one({"_id": id, "email": current_user["email"]}, {"$set": expense})

        if update_result.modified_count == 1:
            if (
                updated_expense := expense_collection.find_one({"_id": id, "email": current_user["email"]})
            ) is not None:
                return updated_expense

    if (existing_expense := expense_collection.find_one({"_id": id, "email": current_user["email"]})) is not None:
...
def delete_expense(id, current_user: User = Depends(get_current_active_user)):
    expense_to_delete: Expense = expense_collection.find_one({"_id": id, "email": current_user["email"]})
    ...
    delete_result = expense_collection.delete_one({"_id": expense_to_delete["_id"], "email": current_user["email"]})
...
def limited_expenses(limit: int = 10, offset: int = 0, current_user: User = Depends(get_current_active_user)):
    if (
        all_expenses := expense_collection.find({"email": current_user["email"]}, limit=limit, skip=offset).sort(
            [("date", pymongo.DESCENDING), ("updated_at", pymongo.DESCENDING)]
        )
...
def ranged_expenses(start_time: date, end_time: date, current_user: User = Depends(get_current_active_user)):
    if (
